...
Kerwin research currently uses three complementary layers:
...
### 3. AI Infrastructure Economics & Capital Cycle Framework
...
→ IT MW / Compute
→ Utilization / GPUh / Token economics
...
Examples of currently useful but **non-permanent** analytical units include:
...
For accounts with authorized access to Kerwin's private research repository, the required procedure is:

```text
read current framework registry
→ fetch current canonical framework
→ fetch dated benchmarks if relevant
→ analyze
```

For accounts without private access, use this Passport for orientation and the public Signal-Chain repository for the public methodology. Do not assume this Passport contains every current private calibration parameter.
