# Kerwin AI Investment Skill — Community Edition v0.2.0

> 不是告诉 AI 应该得出什么结论，而是教它如何形成可验证的投资判断。
>
> 从你熟悉的一家公司，进入全球 AI 产业链。

这是 Kerwin AI Investment Skill 的公开 Community Edition 源码目录。它将财报、公司新闻、产业变化与技术迭代转化为一套可重复的研究程序：

`Reality → Global Positioning（如适用） → Mechanism → Ledger → Economics → Capital → Judgment → Falsification → Next Watchpoint`

## 产品页与下载

产品说明、WorkBuddy 安装包、Starter Prompts 与永久二维码：

https://enyaclawd.com/kerwin-ai-investment-skill/

当前版本：`v0.2.0`

## 适用范围

- 财报与电话会
- 公司新闻、订单、融资、产能与产品变化
- AI Infrastructure / NeoCloud
- Enterprise Agent / Software
- Physical Agent / Robotics
- A 股、港股与其他区域市场 AI 公司的全球产业链定位

核心原则：**Global Ontology, Local Reality｜全球产业框架，本地经营现实。**

## 边界

这不是荐股系统、自动交易系统或保证收益产品。公开版包含方法、空白账本和使用引导，不包含 Kerwin 的私有组合、实时 Thesis Registry、专有 benchmark、客户数据或内部自动化。
